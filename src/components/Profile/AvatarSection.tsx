...
export default function AvatarSection({
  ...
  return (
    <div className="group relative rounded-2xl p-6 md:p-7 bg-black/60 border border-blue-900/40 shadow-[0_10px_30px_rgba(59,130,246,0.15)] backdrop-blur-md overflow-hidden hover:shadow-[0_20px_50px_rgba(59,130,246,0.3)] transition-all duration-500 flex items-center justify-between gap-6">
      ...
      <div className="absolute -inset-1px bg-linear-to-r from-blue-400 via-blue-500 to-blue-600 rounded-2xl -z-10 blur-sm opacity-0 group-hover:opacity-50 transition-all duration-500" />
      ...
      <div className="relative z-10 flex items-center gap-6">
        <div className="relative h-16 w-16 rounded-2xl bg-blue-950/60 border-2 border-blue-900/50 group-hover:border-blue-500/70 overflow-hidden transition-all duration-500 group-hover:scale-105">
          {avatarPreview ? (
            ...
            <div className="flex h-full w-full items-center justify-center text-xl font-black bg-linear-to-br from-blue-500 to-blue-600 text-transparent bg-clip-text">
              {personalForm?.name?.charAt(0).toUpperCase() ?? "U"}
            </div>
          )}
          <div className="absolute inset-0 bg-linear-to-r from-blue-400/30 to-blue-500/30 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-sm" />
        </div>
...
\        <div className="absolute inset-0 bg-linear-to-r from-blue-400/20 to-blue-500/20 opacity-0 group-hover/upload:opacity-100 transition-opacity duration-300 rounded-full blur-sm" />
        <input
          type="file"
          accept="image/*"
          className="hidden"
